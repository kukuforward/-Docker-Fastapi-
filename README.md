# 利用Docker部署FastAPI在云服务器（文本生成与语音转换应用）用于语音对话（语音对话需在本地实时语音转文本）
...
```
## 本地检测示例
    test.py 注意如果要制定关键词检测后回应，需要调用上面的文本转语音api生成本地文件，在检测到关键词时播放
    检测时间建议为2-3秒，不然等待时间过长
    其中test.py 'hello.wav' 是检测到关键词播放音频 ， 'work_v.wav' 是工作返回音频
